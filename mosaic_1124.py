...
def compact_save(PATH, file_name, file_names, crf=25, outputPATH='./'):
    ...
    모자이크 용량 줄이는 함수
    '''
    if os.path.isfile(PATH+file_name+'.mp3'): os.remove(PATH+file_name+'.mp3')  # 원본 음성 삭제
    if os.path.isfile(outputPATH+'The_'+file_names): os.remove(outputPATH+'The_'+file_names)
...
def mosaic(url, yolo_type='yolov5n', target_size=480, gpu=0, min_face=0, conf_thres=0.3, iou_thres=0.5, sigma=55):
    ...
    compact_save(PATH, file_name, file_names, crf=25, outputPATH='C:/Users/USER/Desktop/')

    if os.path.isfile(PATH+file_names): os.remove(PATH+file_names)
    if os.path.isfile(PATH+'ing_'+file_names): os.remove(PATH+'ing_'+file_names)


url = r'C:\Users\USER\Desktop\vs코드\wkit_vs\study\data\humans_pixabay.mp4'
